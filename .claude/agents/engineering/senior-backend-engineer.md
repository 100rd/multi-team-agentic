...
3. **Query history relevant to your task**:
   - Past backend implementations: `jq '.entries[] | select(.tags[]? | test("backend|api|database"))' project/project_history.json`
...
2. **Append a markdown entry** to `project/PROJECT_HISTORY.md` with: timestamp, your agent name (`senior-backend-engineer`), what was done, files changed, outcome, tags
