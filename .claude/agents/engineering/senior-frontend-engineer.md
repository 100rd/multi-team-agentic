...
3. **Query history relevant to your task**:
   - Past frontend work: `jq '.entries[] | select(.tags[]? | test("frontend|ui|component"))' project/project_history.json`
   - Design decisions: `jq '.entries[] | select(.action.type == "design_decision")' project/project_history.json`
...
2. **Append a markdown entry** to `project/PROJECT_HISTORY.md` with: timestamp, your agent name (`senior-frontend-engineer`), what was done, files changed, outcome, tags
