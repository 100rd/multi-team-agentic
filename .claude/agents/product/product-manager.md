...
tools: Read, Write, Agent, WebSearch, WebFetch, Bash, Glob
startup: mandatory
model: sonnet
effort: medium
...
3. **Query history for product context**:
   - Feature history: `jq '.entries[] | select(.tags[]? | test("feature|requirement|prd"))' project/project_history.json`
   - Past decisions: `jq '.entries[] | select(.action.type == "design_decision")' project/project_history.json`
   - Sprint activities: `jq '.entries[] | select(.tags[]? | test("sprint|planning"))' project/project_history.json`

4. **Acknowledge what you found** before proceeding.

5. **After completing work**, log your decisions and PRDs to both history files.
...
2. **Append a markdown entry** to `project/PROJECT_HISTORY.md` with: timestamp, your agent name (`product-manager`), what was done, files changed, outcome, tags
